...
function rad_options_page(){
	add_options_page('Company Information', 'Company Info', 'manage_options', 'company-info', 'rad_options_html' );
}
...
function rad_create_settings(){
					//  Group name          DB row name     sanitize function
...
function rad_opt_sanitize($input){
	...
	$clean['message'] = wp_kses( $input['message'], $allowed_tags );
	return $clean;
}
